...
public class ImageProcessor {
    ...
    public Mat drawBoxes(List<Classifier.Recognition> boxes, double confidenceThreshold){
        ...
        for (Classifier.Recognition box : boxes) {
            ...
            if ((box.getTitle().equals("TBbacillus")) ) {
...
//                color.val[2] = (color.val[2] + 45) % 255;
    ...
    public void loadImage(Bitmap loadedImage, int yoloWidth, int yoloHeight) {
        ...
        //this.widthRatio = (float)loadedImage.getWidth();
       // this.heightRatio = (float)loadedImage.getHeight();

        //int newh = yoloWidth;
        //float neww= (loadedImage.getWidth()* yoloHeight)/loadedImage.getHeight();

       // float x_offset = (float)((yoloWidth - neww) / 2. / yoloWidth);
        //float x_scale = (float)((neww) / yoloWidth);
        //float y_offset = (float) ((yoloHeight - newh) / 2. / yoloHeight);
        //float y_scale = (float)((newh) / yoloHeight);
        ///box.getLocation().left = int((box.getLocation().left - x_offset) / x_scale * widthRatio)


        //box.getLocation().left * widthRatio

        Log.i(TAG, "width of loaded image : " + (float)loadedImage.getWidth());
        Log.i(TAG, "height of loaded image : " + (float)loadedImage.getHeight());
